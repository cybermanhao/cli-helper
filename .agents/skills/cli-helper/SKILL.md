---
name: cli-helper
purpose: 当 CLI Agent 需要人类"看一眼/选一下/确认一下"时，临时拉起浏览器或系统对话框完成交互
description: >
  为 CLI Agent 补充操作系统交互能力的 MCP Server Skill。核心能力是临时拉起浏览器界面让
  人类完成 CLI 无法处理的任务：可视化文件选择（含图片/音频/视频剪辑）、系统对话框确认、
  命令执行前的策略拦截、文件上传、系统通知等。所有 GUI 均为任务触发后临时弹出，任务结束即关闭。
  
  触发场景：
  (1) 用户说"帮我选图片/音频/视频""帮我整理文件""帮我裁剪视频"
  (2) Agent 自主判断需要人类介入：扫描到多个文件需要用户确认删除、多媒体处理前需要用户
      在波形图/时间轴上选区间、危险命令执行前需要二次确认
  (3) 需要操作系统能力：运行命令、打开文件夹、发送系统通知
  (4) 治理场景：命令被 Policy Engine 拦截后需要用户确认或修改策略
---
